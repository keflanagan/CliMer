...
import copy
import json
from utils import common_class, gen_labels, reformat_noun_classes


class Ego4dDatasetTimestamp(Dataset):
    ...
    def get_feature_idxs(self, idx):
        ...
        timestamp = float(self.data['narration_timestamp'][idx])
        video_duration = self.data['video_duration'][idx]
        ...
        if clip_id_before.split('_')[:2] == clip_id.split('_')[:2]:
            if idx == -1:
                ...
                timestamp_before = self.data['narration_timestamp'][actual_idx-1]
            else:
                timestamp_before = self.data['narration_timestamp'][idx - 1]
        ...
        if clip_id_after.split('_')[:2] == clip_id.split('_')[:2]:
            timestamp_after = self.data['narration_timestamp'][idx + 1]
    ...
    def get_first_clip(self, clip1_idx):
        video_ids = self.data['video_id']
        clip_ids = self.data['clip_id']
        narration_pass_list = self.data['narration_source']
        narration_ind_list = self.data['narration_ind']
        verb_class_list = self.data['verb_class']
        noun_classes_list = self.data['noun_class']
        video1_id = video_ids[clip1_idx]
        narration_pass_full_name = narration_pass_list[clip1_idx]  # just used for ensuring the same narration pass
        ...
        return video_ids, clip_ids, narration_pass_list, narration_ind_list, verb_class_list, noun_classes_list,\
            verb_class_clip1, noun_classes_clip1, clip1_id, video1_id, narration_pass_full_name
    ...
    def choose_clips(self, idx):
        ...
        clip1_idx = idx
        video_ids, clip_ids, narration_pass_list, narration_ind_list, verb_class_list, noun_classes_list, \
            verb_class_clip1, noun_classes_clip1, clip1_id, video1_id, narration_pass_full_name = \
            self.get_first_clip(clip1_idx)
        ###############################

        # ensure same video_id and the same narration pass
        same_video_clips = np.where((video_ids == video1_id) & (narration_pass_list == narration_pass_full_name))[0]
    ...
    def get_bert_features(self, clip1_id, clip2_id, clip3_id):
        ...
        with caption_data.begin(write=False) as txn:
            ...
            bert_features1 = clip1["features"].detach()
            ...
            bert_features2 = clip2["features"].detach()
            ...
            bert_features3 = clip3["features"].detach()
